...
_VICTRON_STATE = {0: "off", 1: "low power", 3: "fault", 3: "bulk", 4: "absorption", 5: "float"}
...
async def parse_govee(data: bytes, dev, mac, device):
    ...
    if device:
        ...
        config.set(f"discover/{mac}", {"alias": f"Govee_{mac}", "description": f"Govee T/H {temp/100}C {dev.rssi}dBm"})
        ...
        async with aioble.scan(duration_ms=5000, active=True) as scanner:
            async for dev in scanner:
                for manufacturer, data in dev.manufacturer():
                    logger.info(f"Scan {dev.name()} manuf={manufacturer:04x} {dev.device.addr_hex()}")
